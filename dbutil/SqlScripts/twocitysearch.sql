CREATE DEFINER=`admin`@`%` PROCEDURE `twocitysearch`(city1 VARCHAR(3), city2 VARCHAR(3), city3 VARCHAR(3),
    city4 VARCHAR(3), date1 VARCHAR(10), date2 VARCHAR(10), date3 VARCHAR(10))
BEGIN
select * FROM (
	(select (f1.price + f2.price + f3.price) as total, f1.routeStr as rs1, f1.depTime as dt1, f1.arrTime as at1, f1._id as i1, f1.carrier as c1, f1.price as p1, f1.layovers as l1,
	...
	(select (f1.price + f2.price + f3.price) as total, f1.routeStr, f1.depTime, f1.arrTime, f1._id, f1.carrier, f1.price, f1.layovers,
